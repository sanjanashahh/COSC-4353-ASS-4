...
router.post('/profileData', validateVolunteerForm, async (req, res) => {
    ...
    const { fullName, address1, address2, city, state, zipcode, skills, preferences, availability, summary } = req.body;
    ...
    try {
        ...
        if (existingProfile) {
            return res.status(400).json({
                errors: [{ msg: 'Duplicate profile submissions are not allowed for the same full name.' }],
            });
        }
